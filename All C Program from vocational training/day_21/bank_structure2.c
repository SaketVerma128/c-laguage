...
int main(void)
{
    ...
    for (int i = 0; i < 5; ++i)
    {
        printf("Account Number: %d\nAccount Type: % s\nAccount Holder fist Name : % s\nAccount Holder Last Name : % s\nbalance : % .2lf\n-- -- -- -- -- --\n ", bank_customers[i].account_number, bank_customers[i].account_type, bank_customers[i].account_holder_name.first_name, bank_customers[i].account_holder_name.last_name, bank_customers[i].balance);
    }
}